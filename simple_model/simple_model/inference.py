...
def main():
    ...
    if args.model_type == 'flow':
        if std_norm:
            file_ = np.load('scaling/'+args.name + '_std.npz')
            mu_, std_ = file_['mu'], file_['std']
            add_ = jnp.sum(jnp.log(std_[:3]))
        else:
            file_ = np.load('scaling/'+args.name + '_minmax.npz')
        ...
        flow_model = eqx.tree_deserialise_leaves('weights/'+ args.name + '.eqx', skel_flow)
    ...
    prior_predictions = prior_predictive(rng_key_2, z_s_, dust=False)
    ...
    # --- MCMC Setup & Run ---
    init_dict = {'w': -1.1, 'Om0': 0.35, 'Omde': 0.65, 'M0': -19.35, 'c0_int': -0.04, 'x0': -0.4, 
                 'sigma_int': 0.1, 'sigma_c': 0.05, 'sigma_x': 1, 'alpha': -0.11, 'beta': 3.0, 'alpha_c': -0.01}
    ...
    # --- Save Output ---
    save_name = args.name + '_cmb' if args.cmb else args.name
    dir_out = f"toy_chains_{save_name}"
