...
def main():
    ...
    data_path = 'training_data/'+ args.data if args.data.endswith('.npy') else args.data + '.npy'
    ...
    X = (X - mu) / std
    onp.savez(f"scaling/{args.name}_std.npz", mu=onp.array(mu), std=onp.array(std))
    ...
    # Final save
    eqx.tree_serialise_leaves(f"weights/{args.name}.eqx", eqx.combine(params, static))
