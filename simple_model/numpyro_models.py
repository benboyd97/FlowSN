...
def sample_model(z_s, d_hat_s=None, log_x_err=None, log_c_err=None, log_mag_err=None, cov_m_c=None, cov_c_x=None, 
                 cov_m_x=None, sel_s=None, M0=-19.365, sigma_res=0.1, alpha=-0.14, alpha_c=-0.008, beta=3.1, 
    ...
    with numpyro.plate("plate_i", n_sne):
        ...
        m_s_ = numpyro.sample("m_s_", dist.Normal(M0 + mu_s + alpha * x_s_ + beta * c_s_, sigma_res))
...
def mcmc_model(z_s, data_s=None, data_err_s=None, h=H0/100, m_cut=24, sigma_cut=0.25, a=-0.1, b=-1, 
               sigma_pec=300, z_s_err = 0, wCDM=True, model_type='analytical', 
    ...
    with numpyro.plate("plate_i", n_sne):
        ...
        err2 = mu_vpec_grad_vmap(z_s,jnp.zeros(n_sne),jnp.zeros(n_sne))*sigma_pec/299792.458
        cov =  mu_grad_vmap(z_s,jnp.zeros(n_sne),jnp.zeros(n_sne))*mu_vpec_grad_vmap(z_s,jnp.zeros(n_sne),jnp.zeros(n_sne))*(sigma_pec/299792.458)**2
        ...
        if model_type == 'analytical':
            numpyro.sample("obs", SkewNormalPlus3D(mu_s + M0 + eps, jnp.repeat(c0, n_sne), jnp.repeat(x0, n_sne), sigma_res, sigma_c, sigma_x,
            ...
            numpyro.sample("obs", Naive(mu_s + M0 + eps, jnp.repeat(c0, n_sne), jnp.repeat(x0, n_sne), sigma_res, sigma_c, sigma_x,
                       alpha, beta, alpha_c, data_err_s[:,0], data_err_s[:,1], data_err_s[:,2], data_err_s[:,3], data_err_s[:,4], data_err_s[:,5], m_cut, sigma_cut, a, b), sample_shape=(1,), obs=data_s)
        elif model_type == 'flow':
            # This matches the signature in the new __init__ above
            numpyro.sample("obs", FlowSNP3D(
                mu_s + M0 + eps, jnp.repeat(c0, n_sne), jnp.repeat(x0, n_sne), 
                jnp.repeat(sigma_res, n_sne), jnp.repeat(sigma_c, n_sne), jnp.repeat(sigma_x, n_sne),
