...
def hubble_distance(H0, *, xp=np):
    ...
    """
    c_km_per_s = constants.get("c_km_per_s", xp)
    return xp.asarray(c_km_per_s) / H0
...
def z_at_value(func, fval, zmin=1e-4, zmax=100, **kwargs):
    ...
    zs = xp.exp(xp.linspace(xp.log(zmin), xp.log(zmax), 1000))
    xs = func(zs, **kwargs)
    values = jnp.interp(fval, xs, zs, left=zmin, right=zmax, period=None)
